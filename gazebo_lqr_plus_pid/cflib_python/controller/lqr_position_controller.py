...
class LQRPositionController:
    ...
    def __init__(self):
        ...
        self.max_roll_rad = math.radians(10.0)
        self.max_pitch_rad = math.radians(10.0)
        self.max_az = 15.0
    ...
    def position_controller(self, setpoint: Setpoint, state: State) -> Tuple[float, Attitude]:
        ...
        # IMPORTANT:
        # During takeoff, disable horizontal attitude commands.
        # Otherwise tiny x/y drift during the vertical ramp can excite the
        # attitude loop before the drone is safely near hover height.
        has_horizontal_target = (
            abs(setpoint.position.x) > 1e-4
            or abs(setpoint.position.y) > 1e-4
        )

        has_horizontal_error = (
            abs(ex) > 1e-3
            or abs(ey) > 1e-3
        )

        horizontal_enabled = (
            (has_horizontal_target or has_horizontal_error)
            and state.position.z > 0.5
        )
