...
class LQRPositionController:
    ...
    def position_controller(self, setpoint: Setpoint, state: State) -> Tuple[float, Attitude]:
        ...
        # X-only horizontal test.
        # During takeoff, disable horizontal feedback completely.
        # Only enable pitch control after the vehicle is close to hover altitude.
        roll_cmd = 0.0
        if state.position.z < 0.90 * setpoint.position.z:
            pitch_cmd = 0.0
        else:
            pitch_cmd = float(u[1])
        az_cmd = float(u[2])
